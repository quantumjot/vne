...
class InfinitePaddedImage:
    ...
    def __init__(
        ...
        image: npt.NDArray,
        *,
        mode: Union[str, PaddingMode] = "none",
        ...
        self._data = image
        self._mode = (
            PaddingMode(mode.lower()) if isinstance(mode, str) else mode
        )
    ...
    def __getitem__(self, coords: tuple) -> npt.NDArray:
    ...
    def _get_pixel(self, coords) -> npt.NDArray:
    ...
    def _get_slice(self, r_coords) -> npt.NDArray:
    ...
    def _pad(self, coords) -> npt.NDArray:
        ...
        for dim, mesh in enumerate(meshes):
            dir = (
                -2 * np.mod(mesh // self.shape[dim], 2) + 1.0
            )  # convert to +1/-1
